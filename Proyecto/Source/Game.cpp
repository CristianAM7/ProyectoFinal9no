...
void CGame::JugandoPintar(){
	...
	for (int i = 0; i < nivel[nivelActual].Enemigos_VisiblesAlMismoTiempo; i++)
	{
		...
		if (enemigoArreglo[i]->Colision(nave, Nave::TipoColision::NAVE) || enemigoArreglo[i]->Colision(nave, Nave::TipoColision::BALA))
			vida--;
		if (nave->Colision(enemigoArreglo[i], Nave::TipoColision::BALA)){
...
void CGame::JugandoActualizar(){
	...
	if (keys[SDL_SCANCODE_C]){//bala propia / nave enemigo
	...
	if (keys[SDL_SCANCODE_V]){//nave propia / nave enemigo
